 ...
 * introduction: the implemented functions are postfix_traversal, rotate, recut, swap_topology, swap_module, is_internal_node, is_leaf_node,
 * is_in_subtree, get_total_resources and init_slicing_tree.
 * In postfix_traversal, the code goes to the very left-most of the tree and initializes that as the first element in the array expressions.
 * It will then travel in post-order (left, right then root) until it fills out all of the expressions array with the modules and cutline 
 * nodes. 
 * In rotate, the width and height of a module is switched.
 * In recut, the cutlines are swapped so that horizontal cutlines become vertical and vertical cutlines become horizontal.
 * In swap_topology, two nodes have their parent pointers switched so their new roots are each other's old roots. The parent's pointers are
 * also changed accordingly. 
 * In swap_module, the pointers to modules for two nodes are exchanged to that they now point to one another's modules.
 * In is_internal_node, it checks to see if the node is a cutline node or not since cutline nodes can only be internal because they have to 
 * point to some child.
 * In is_leaf_node, it checks if a node is a leaf, or if it points to no children.
 * In is_in_subtree, the function checks down the subtree of one node to see if another node is in it.
 * In get_total_resources, this function finds the module nodes and tallies up all of the resources they take up for the whole tree.
 * In init_slicing_tree, it creates a tree with the modules in a provided array. However, the right nodes of the tree must be a module and a 
 * leaf node while the left child will be a vertical cutline node until it reaches the very last two modules (in which they will be leaf nodes).
 */
// Function: is_leaf_node
// Return 1 if the given slicing tree node is a leave node, and 0 otherwise.
int is_leaf_node(node_t* ptr) {
if (ptr->left == NULL && ptr->right == NULL)			//checks if the node has a left and right child
	return 1;						//no left or right child found, this is a leaf node
else								
...
if (ptr->cutline == H || ptr->cutline == V)		//check to see if the cutline of the node is either horiztonal or vertical
	return 1;					//there is a cutline associated with the node, return 1
else						
	return 0;					//no cutline associated: return 0
}
...
//base case: we are no longer in a node/tree is empty
if (a == NULL)
	return 0;
//base case: we are on the node b
if (a == b)				
	return 1;
//recursive: check through the rest of the tree
if(is_in_subtree(a->left, b))		//check left branch. If b is found under a, return 1 immediately
	return 1;		
if (is_in_subtree(a->right, b))		//check right branch. If b is found under a, return 1
	return 1;
return 0;
}
...
if (!is_leaf_node(ptr))				//checks to make sure that we are given a leaf node
	return;					//does nothing if we are not a leaf node
int temp = ptr->module->w;			//stores the width of the module
ptr->module->w = ptr->module->h;		//stores the height into the width member of the module
ptr->module->h = temp;				//stores the width into the height of the module
}
...
assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);	//ensures that ptr is a internal node
if (ptr->cutline == H)							//if the cutline is 1 or horizontal
	ptr->cutline = V;						//change the cutline to a vertical one
else									//if the cutline was not 1 to begin with (it is vertical)
	ptr->cutline = H;						//change the cutline to horizontal
  return;
}
...
module_t* temp = a->module;					//sets up a temporary pointer value that points to the module in a
a->module = b->module;						//assign the ptr to the module for node b to node a
b->module = temp;						//assign the ptr to the module for node a to node b
}
...
if (b->parent->left == b)		//check if b is a left child (check to know if it is a left/right child is from lab11)
	b->parent->left = a;		//put pointer of b's parent to a
else if (b->parent->right == b)		//check if b is a right child
	b->parent->right = a;		//put pointer of b's parent to a
if (a->parent->left == a)		//check if a is a left child
	a->parent->left = temp;		//put pointer to a's parent to b
else if (a->parent->right == a)		//check if a is a right child
	a->parent->right = temp;	//put pointer to a's arent to b
node_t* b_parent = b->parent;		//node down the ptr to b's parent
b->parent = a->parent;			//assign a's parent to b
a->parent = b_parent;			//assign b's parent to a
}
...
//base case: 
	if(ptr == NULL) return;					//if there is no node, do not input any information in
	postfix_traversal(ptr->left, nth, expression);		//recursion to the left
	postfix_traversal(ptr->right, nth, expression);		//recursion to the right
	if (ptr->module != NULL)				//check if the pointer has a module or not
	{	//ptr points to a module
		(expression + *nth)->module = ptr->module;		//put the module of the pointer into the expression array at the nth index
		(expression + *nth)->cutline = UNDEFINED_CUTLINE;	//because we are a module, the cutline needs to be set as undefined
	}
	else
	{	//ptr is not a module or it is a cutline	
		(expression + *nth)->cutline = ptr->cutline;		//put the cutline of the pointer into the array at the nth index
		(expression + *nth)->module = NULL;			//because this is a cutline node, the module is NULL or not there
	}
	*nth = *nth + 1; 						//increment the value of nth
}
...
{ 
//base case: 
if (ptr == NULL)			//reached a node that does not exist. Because it is not a node, return 0 for the number
	return 0;			//of resources
if (ptr->cutline == UNDEFINED_CUTLINE)	//ensure that we are at a node with a module
{
	return ptr->module->resource;	//return the amount of resources at that one node
}
//recursive case
return get_total_resource(ptr->left) + get_total_resource(ptr->right);		//adds up all the resources to the left of the root
										//node and all the resiurces to the right of it
}
...
assert(n >= 0 && n < num_modules);
	node_t* curr = (node_t*) malloc(sizeof(node_t));		//initialize a location for node named curr
	curr->parent = par;						//set the parent of curr to the ptr paramenter "par"
	if (n == num_modules - 1)					//check if there is only one module left (meaning we have to make a left leaf node) 
	{
		curr->module = modules+n;				//set the curr's ptr to the module to modules offset n
		curr->cutline = UNDEFINED_CUTLINE;			//because this is a module, set the cutline to undefined
		curr->right = NULL;					//set curr's right ptr to NULL
		curr->left = NULL;					//set curr's left ptr to NULL because it is a leaf node
		return curr;						//return the address of this last leaf node (left leaf node)
	}	
	curr->cutline = V;						//root's cutline is vertical
	curr->module = NULL;						//the root will always be a vertical cut.
									// Therefore it has no modules
	node_t* r = (node_t*) malloc(sizeof(node_t));			//allocate a space for the right child node
	curr->right = r;						//set the par's right child to this allocated space

	r->parent = curr;					//set the parent of r to be curr
	r->cutline = UNDEFINED_CUTLINE;				//set the cutline for the right node to be undefined (has to be a module leaf node)
	
	r->module = modules+n;					//set the module to the same address as the module in the modules array but offset by n

	r->left = NULL;						//set the right child of par to be a leaf node
	r->right = NULL;
	
	curr->left = init_slicing_tree(curr, n+1);		//setting the left node to whatever the previous recursion's returned for the left node 
	
	return curr;						//set the current node that it just made to be the "left node" of the outer recursive statement
}
